...
import numpy as np
import json
from models.layers.mesh_ring import Ring
###############################################################################
# Helper Functions
###############################################################################
...
from models_seg.segformer import Segformer
...
class PriorNet(nn.Module):
    ...
    def __init__(self, n_edges, in_ch=6, convs=[32, 64], pool=[], res_blocks=0,
        ...
        self.encoder_decoder =Segformer(
        ...
        with open('encoder_decoder_params.json', 'w', encoding='utf-8') as json_file:
            json.dump(encoder_decoder_dict, json_file, ensure_ascii=False, indent=4)
    ...
    def forward(self, x, meshes):
        ...
        x =self.ring(x, meshes_new)
        #f1 = FlopCountAnalysis(self.encoder_decoder,x)
        #print(f1.total())
