...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.sr_ratio_int = sr_ratio
        sr_ratio_int = sr_ratio
        self.sr_ratio = (1,sr_ratio)
        if sr_ratio_int > 1:
...
class Segformer(nn.Module):
    ...
    def forward(self, x):
        ...
        original_size = (x.size(2), 1)
        # Extract multi-scale features from the Transformer encoder
        ...
        _c4 = F.interpolate(_c4, size=original_size, mode='bilinear', align_corners=False)
    
        _c3 = self.linear_c3(c3).permute(0, 2, 1).reshape(n, -1, c3.shape[2], c3.shape[3])
        _c3 = F.interpolate(_c3, size=original_size, mode='bilinear', align_corners=False)
    
        _c2 = self.linear_c2(c2).permute(0, 2, 1).reshape(n, -1, c2.shape[2], c2.shape[3])
        _c2 = F.interpolate(_c2, size=original_size, mode='bilinear', align_corners=False)
    
        _c1 = self.linear_c1(c1).permute(0, 2, 1).reshape(n, -1, c1.shape[2], c1.shape[3])
        _c1 = F.interpolate(_c1, size=original_size, mode='bilinear', align_corners=False)
